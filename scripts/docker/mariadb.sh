...
if [ ! -d "$DATADIR/mysql" ]; then
	...
	if [ ! -z "$MYSQL_USER" -a ! -z "$MYSQL_USER_PASSWORD" -a ! -z "$MYSQL_ROOT_HOST" -a "$MYSQL_ROOT_HOST" != 'localhost' ]; then
		read -r -d '' userCreate <<-EOSQL || true
			CREATE USER '${MYSQL_USER}'@'${MYSQL_ROOT_HOST}' IDENTIFIED BY '${MYSQL_USER_PASSWORD}' ;
			GRANT ALL ON ${MYSQL_USER}.* TO '${MYSQL_USER}'@'${MYSQL_ROOT_HOST}' WITH GRANT OPTION ;
		EOSQL
	fi
